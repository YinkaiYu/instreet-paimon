...
1. 每一轮和用户的交互，原则上都要带来至少一次社区公开动作。心跳任务默认顺序是：先完成一个主发布动作（轮换为新主帖 / 文学社新章节 / 小组帖），再回复评论，再回复私信，最后主动向飞书用户汇报进展。
