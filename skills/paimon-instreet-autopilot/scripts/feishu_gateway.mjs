...
function buildPlanExecutionInput(planText, triggerSource = "card") {
  ...
  return [
    ...
    "不要再把整份计划复述一遍，先从当下最强的压力点动手；如果现场和原计划顺序冲突，就直接改写起手顺序，并继续用飞书短句同步进度。",
...
function buildCodexPrompt(
  ...
  return [
    ...
    "派蒙，你正在通过飞书和用户连续协作。",
    "先把 AGENTS.md 当最高记忆入口，再按需读取 paimon-instreet-autopilot / paimon-freedom skill、相关 references 和 state/current 下的实时状态。",
    "config/paimon.json 是运行配置，不是人格来源、选题理由或研究入口。",
    ...
    "默认只给一条现在最值得立刻执行的下一步；只有用户明确要多个点时，才补充 2 个最值得继续测或继续追的点。",
    ...
    "不要把通知数、队列数、快照字段本身误写成题目；只有当它们已经咬住失败链、外部样本或制度冲突时，才值得升成公开判断。",
    "每一轮和用户的交互，原则上都要带来至少一次社区公开动作。不要写死公开动作顺序；这轮最强的压力点落在哪，就从哪条公开线起手，主帖、章节、小组帖、评论维护或高质量外部评论都可以。",
    "如果当前没有够格的新主帖 bundle，就不要硬补空心 theory/tech 标题；直接去做评论维护、外部切入或修复链也算有效公开动作。"
  ].join("\n");
}
